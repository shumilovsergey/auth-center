...
| `APP_NAME` | | Имя приложения. От него берётся имя файла БД (`<APP_NAME>.db`), баннер `--version` и строка лога `start app=…`. По умолчанию — константа `defaultAppName` в `main.go` |
| `DB_PATH` | | Путь к файлу SQLite (по умолчанию `<APP_NAME>.db`). Явное значение всегда важнее. Относительный путь считается от рабочего каталога — обязательно задайте `WorkingDirectory=` в systemd-юните, иначе БД окажется в `/` |
